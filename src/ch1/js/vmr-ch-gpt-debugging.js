  ...
  googletag.pubads().addEventListener('slotVisibilityChanged', function(event) {
    ...
    if (slot.getTargeting('refresh').indexOf('true') > -1) {
      ...
      if (slot && minInViewPercentage <= event.inViewPercentage) { // visible
        ...
        console.log('Slot ' + advert + ' is hidden');
...
function clearTimers() {
  ...
  console.log('Refresh timers have been cleared');
}

// Set a new refresh timer
function setTimer(slot,advert) {
  refreshTimers.push(setInterval(slotRefresh, refreshInterval * 1000, slot, advert));
  console.log('Refresh timer was set for ' + advert);
}

// The slot refresh function
function slotRefresh(a,b) {
  // only fire the actual refresh if user is active
  if (userIsActive()) {
    googletag.pubads().refresh([a]);
    console.log('Slot ' + b + ' was refreshed');
  }
}
